...
The warning tell us, there are 177 missing value. So, let's imputing those missing age values. One way to do this is to create a model that predicts the ages based on other variables. There are many package can do the prediction, such as rpart(recursive partitioning for regression), and mice(Multivariate Imputation by Chained Equations). [reference for mice](http://www.jstatsoft.org/article/view/v045i03/v45i03.pdf)
Let's try the mice: 
```{r}
...
# levels(train$Fsize) <- c("solo","double","Fsize_3","Fsize_4","large_family")
# fit the glm with Fsize: 
Survived_model1 <- glm(train$Survived ~ train$Title, family=binomial)
summary(Survived_model1)
anova(Survived_model1, test="Chisq")
```

### 4.6 SLLR on Survived~Age + Sex
...
###  SLLR on Survived~Age + Title + Sex
```{r echo=TRUE}
...
### 4.7 SLLP with interaction terms
